...
// 1,000,000 [us/ s] / 208 [sps]
#define MICROS_PER_SAMPLE  4804
...
void setup() {
  ...
  if ( adc.available ) { 
    ...
    error = adc.regularspeed();
    if ( error == 0 ) { Serial.println("Sensor: regularspeed set."); } 
    else              { Serial.printf("Sensor: failed to set regularspeed. Error %d\n", error); }
...
void loop() {
  ...
  if (adc.available) {
      error = adc.read(&voltage, false); // do not send stop
      if (error == 0) { 
        Serial.printf("V: %0.5f, SPS: %d\n", voltage, rate);
      } else {
        Serial.printf("Could not read sensor. Error %d\n", error); 
      } 
  }
